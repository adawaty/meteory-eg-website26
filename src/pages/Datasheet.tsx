...
export default function Datasheet() {
  ...
  const specTable = language === "ar" && product.specTableAr ? product.specTableAr : product.specTable;
    ...
    <div className="min-h-screen bg-muted/20 p-8 print:p-0 print:bg-white">
      ...
      <div className="max-w-[210mm] mx-auto mb-8 flex justify-between items-center print:hidden">
        ...
        <div className="flex gap-2">
          {product.datasheet && (
            <a href={product.datasheet} download target="_blank" rel="noreferrer">
              <Button variant="outline">
                <Download className="mr-2 h-4 w-4 rtl:ml-2 rtl:mr-0" /> {t("common.download")}
              </Button>
            </a>
          )}
      ...
      <div className="max-w-[210mm] mx-auto bg-white shadow-xl print:shadow-none p-[15mm] min-h-[297mm] flex flex-col relative overflow-hidden">
        ...
        <header className="flex justify-between items-center border-b-4 border-primary pb-6 mb-8">
          <img src={language === "ar" ? logoFullAr : logoFullEn} alt="Meteory" width={240} height={64} className="h-16 w-auto object-contain" />
          ...
          <div className="grid grid-cols-3 gap-8 mb-8">
            <div className="col-span-1">
              <div className="border border-gray-200 p-4 bg-gray-50 flex items-center justify-center h-full">
                <img src={product.image} alt={product.name} width={800} height={800} loading="lazy" className="max-h-64 w-auto object-contain" />
              </div>
            </div>
                  ...
                  <Table>
                    <TableBody>
                      {Object.entries(specTable).map(([k, v]) => (
                        <TableRow key={k} className="border-b border-gray-200 last:border-0 odd:bg-white even:bg-gray-50">
                          <TableCell className="font-bold border-r border-gray-200 w-2/5">{k}</TableCell>
                          <TableCell className="w-3/5">{v}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -z-10 opacity-[0.03] pointer-events-none">
          <img src={language === "ar" ? logoFullAr : logoFullEn} className="w-[150mm]" alt="" width={567} height={180} loading="lazy" />
        </div>
      </div>
    </div>
  );
